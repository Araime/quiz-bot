...
`questions`. Скрипт циклом проходит по указанной папке, если файл имеет формат 
`txt` и в кодировке `UTF-8`, он открывается и его содержимое добавляется в общий 
список вопросов. Если файл будет отличаться по структуре, вам придётся переписать 
